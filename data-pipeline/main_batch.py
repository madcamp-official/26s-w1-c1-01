...
DB 반영 단계는 건너뛰고(스크래핑이 못 만든 오래된 CSV를 다시 밀어넣지 않기 위해)
나머지 카테고리는 계속 진행한다.

실행 순서:
  1. 환율: exchange_scrapers.py(API -> exchange.csv) -> build_currencies.py(CSV -> currencies upsert)
  2. 항공권: flight_scraper.py(스크래핑 -> flight_prices.csv) -> build_flights.py(CSV -> flight_price_scrapes upsert)
  3. 숙박: stay_scraper.py(스크래핑 -> stay_prices.csv) -> build_stay.py(CSV -> stay_price_scrapes upsert)
...
def main():
    if run_step("환율 스크래핑 (exchange_scrapers)", exchange_scrapers.main):
        run_step("환율 DB 반영 (build_currencies)", build_currencies.main)

    flight_ok = run_step("항공권 스크래핑 (flight_scraper)", flight_scraper.main)
    if flight_ok:
        run_step("항공권 DB 반영 (build_flights)", build_flights.main)

    stay_ok = run_step("숙박 스크래핑 (stay_scraper)", stay_scraper.main)
    if stay_ok:
        run_step("숙박 DB 반영 (build_stay)", build_stay.main)
...
def run_for_city(city_id):
    ...
    flight_ok = run_step(f"[{city_id}] 항공권 스크래핑 (flight_scraper)", lambda: flight_scraper.main(city_id))
    if flight_ok:
        run_step(f"[{city_id}] 항공권 DB 반영 (build_flights)", lambda: build_flights.main(city_id))

    stay_ok = run_step(f"[{city_id}] 숙박 스크래핑 (stay_scraper)", lambda: stay_scraper.main(city_id))
    if stay_ok:
        run_step(f"[{city_id}] 숙박 DB 반영 (build_stay)", lambda: build_stay.main(city_id))
