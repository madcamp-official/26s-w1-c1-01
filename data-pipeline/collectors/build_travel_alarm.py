...
여행경보 API는 국가를 ISO 3166-1 alpha-3(iso_code)로 식별하므로, country_id(alpha-2)와
연결하려면 iso3 매핑이 필요하다. 예전에는 외교부 국가/지역별 표준코드 원본 CSV를
읽었지만, 그 CSV는 data/raw/ 아래에 있어 git에 커밋되지 않는다(.gitignore 처리) -
저장소를 새로 clone한 환경에서는 파일이 없어 이 스크립트가 곧바로 실패했다.
그래서 그 매핑을 countries.iso3 컬럼(마이그레이션 add_countries_iso3로 추가, 기존
CSV에서 백필)에 미리 심어두고 여기서는 DB에서 조회한다 - 로컬 파일 의존성이 없다.
countries 테이블에 이미 있는 country_id만 UPDATE 대상이라(INSERT는 하지
...
def load_iso3_to_country_id(conn=None):
    connection, owns_conn = get_connection(DB_URL, conn)
    try:
        with connection.cursor() as cur:
            cur.execute("SELECT country_id, iso3 FROM countries")
            rows = cur.fetchall()
    finally:
        if owns_conn:
            connection.close()

    return {iso3: country_id for country_id, iso3 in rows}


def build_rows(conn=None):
    iso3_to_country_id = load_iso3_to_country_id(conn=conn)
