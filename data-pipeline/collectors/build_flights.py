...
from _scrape_log import upsert_scrape_log
...
def main(df, city_id=None, conn=None):
    """df는 flight_scraper.main()이 반환한 DataFrame. city_id를 주면 해당 도시 로그만 upsert한다.

    conn을 주면 커넥션을 재사용한다(main_batch.run_for_city 참고). 여러 도시를 한 번에
    스크랩한 df가 들어와도(예: 재시도로 같은 도시가 두 번 담긴 경우) city_id 필터링 시
    마지막 행이 가장 최근 스크랩이므로 그것만 쓴다.
    """
    return upsert_scrape_log(
        df, DB_URL, CREATE_TABLE_SQL, CREATE_INDEX_SQL, UPSERT_SQL,
        nullable_cols=["airline", "source_url"],
        table_label="flight_price_scrapes",
        city_id=city_id, conn=conn,
    )
